...
float Transform::dot(const Vec &a, const Vec &b) {
	float result;
...
float Transform::det3x3(const Mat4 & m) {
	float d = 0.0f;
...
float Transform::det(const Mat4 & m) {
...
float Transform::trace(const Mat4 & m) {
	int i;
	float sum = 0.0f;
	for (i = 0; i < 4; ++i) {
		sum += m.mat[i][i];
...
float Transform::trace3x3(const Mat4 & m) {
	int i;
	float sum = 0.0f;
	for (i = 0; i < 3; ++i) {
		sum += m.mat[i][i];
