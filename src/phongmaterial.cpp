...
PhongMaterial::PhongMaterial(const Vec &ambient, const Vec &diffuse,
                             const Vec &specular, double shininess,
                             const Vec &emission)
    : m_ambient(ambient), m_diffuse(diffuse), m_specular(specular),
      m_emission(emission), m_shininess(shininess) {}

bool PhongMaterial::calc_scattered_ray(const Ray &ray,
                                       const IntersectionInfo &hit_info,
                                       Vec &atten_factor,
                                       Ray &scattered) const {
  Vec reflected_dir =
      ray.direction() -
      2.0 * Transform::dot(ray.direction(), hit_info.normal) * hit_info.normal;
...
Vec PhongMaterial::compute_light(double visibility, const Vec &light_color,
                                 const Vec &direction, const Vec &normal,
...
Vec PhongMaterial::calc_lighting(
    const PointLight &point_light, const Vec &eye, const Scene &scene,
  ...
  return compute_light(visibility, atten_light_color, dir,
                       intersection_info.normal, half);
}

Vec PhongMaterial::calc_lighting(
    const DirectionalLight &dir_light, const Vec &eye, const Scene &scene,
    const IntersectionInfo &intersection_info) const {
  ...
  Vec pos_vector(intersection_info.position.x(), intersection_info.position.y(),
                 intersection_info.position.z());
